...
int DetectThread::AddDetect(const vector<struct DetectNode_t> &vNode) {
    mDetectMutex->Lock();
    std::vector<struct DetectNode_t>::const_iterator it = vNode.begin(), eIt = vNode.end();
    std::map<struct DetectNode_t, struct DetectResult_t>::iterator itdel;
    for (; it != eIt; ++it) {
...
int DetectThread::DelDetect(const std::vector<struct DetectNode_t> &vNode) {
    mDetectMutex->Lock();
    std::vector<struct DetectNode_t>::const_iterator it = vNode.begin(), eIt = vNode.end();
    std::map<struct DetectNode_t, struct DetectResult_t>::iterator itdel;
    for (; it != eIt; ++it) {
