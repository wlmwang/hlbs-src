...
RouterServerTask::RouterServerTask(int iNewSocket, struct sockaddr_in *stSockAddr):wTcpTask(iNewSocket,stSockAddr)
{
    //...
}
 ...
 *  发送数据
 */
int RouterServerTask::HandleSendMessage(char *pBuffer, int nLen)
{
 ...
 *  发送数据 daemon
 */
void RouterServerTask::ProcessSendMessage()
{
 ...
 *  处理接受缓冲区数据 daemon
 */
void RouterServerTask::ProcessRecvMessage()
{
	static char szBuff[MAX_PACKAGE_LEN] = {0};
	int iRet;
	while(1)
...
void RouterServerTask::ParseRecvMessage(struct wHeadCmd *pHeadCmd, char *pBuffer, int iLen)
{
