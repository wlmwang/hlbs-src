...
int SvrQos::CleanNode()
{
    ...
    {
    	for (; mapReqIt != mMapReqSvr.end(); mapReqIt++)
    	{
...
int SvrQos::RouteCheck(struct SvrStat_t* pSvrStat, struct SvrNet_t& stNode, double dFirstReq/*此次WRR轮转的分配限度*/, bool bFirstRt)
{
    ...
    */
    ...
    LOG_DEBUG(ELOG_KEY, "[svr] RouteCheck at end return 0 gid=%d xid=%d host:%s port:%hu pre(%d)", stNode.mGid,stNode.mXid,stNode.mHost,stNode.mPort,stNode.mPre);
    return 0;
}
...
int SvrQos::ReqRebuild(const struct SvrNet_t &stSvr, struct SvrStat_t* pSvrStat)
{
	...
	else	//门限收缩(可能原因为：上周期存在过度扩张)，需要按有效失败率收缩门限
	{
