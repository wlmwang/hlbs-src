...
int main(int argc, const char *argv[])
{
	...
	{
		//初始化守护进程
		const char *pFilename = "./router_client.lock";
	...
	*/
	...
	//获取服务器实体
    RouterClient *pClient = RouterClient::Instance();
	if(pClient == NULL) 
	{
		LOG_ERROR("default", "Get RouterClient instance failed");
		exit(1);
	}

	//准备工作
	pClient->PrepareStart("192.168.8.13", 10006);
	...
	//服务器开始运行
	LOG_INFO("default", "RouterClient start succeed");
	pClient->Start();
