...
#ifndef _W_HASHMAP_H_
#define _W_HASHMAP_H_
...
namespace W
{
	...
	class hash_base_node_const_iterator
	{
		...
		private:
			...
			void *next()
			{
				W_ASSERT(ptr_ != NULL, return NULL);
	...
	class hash_map 
	{
...
#ifdef _DEBUG_
				// 绝不可能发生的情况
				W_ASSERT(first_free_pos != -1, return ret);
#endif
...
#ifdef _DEBUG_
						// 绝不可能发生的情况
						W_ASSERT(used_.first_ == -1 && used_.last_ == -1, return ret);
#endif
...
#ifdef _DEBUG_
						// 绝不可能发生的情况
						W_ASSERT(used_.first_ != -1, return ret);
#endif
...
#ifdef _DEBUG_
					// 绝不可能发生的情况
					W_ASSERT(first_bucket_node != -1, return ret);
#endif
					// 添加到桶中链表的第一个位置
					bucket_[pos].first_ = first_free_pos;
		...
		private:
			void erase(iterator it, bool if_find)
			{
				...
				W_ASSERT(array_pos >= 0 && array_pos < count, return);
				...
				if(if_find)
				{
					...
					{
#ifdef _DEBUG_
						W_ASSERT("erase: iterator not used", return);
#endif
						return;
					}
				}
