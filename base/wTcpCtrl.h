...
#ifndef _W_TCP_CTRL_H_
#define _W_TCP_CTRL_H_
...
#include "wTcpListen.h"

template <unsigned int nRecvBufLen, unsigned int nSendBufLen,int iMaxListenNum, int iMaxClientNum, int iMaxAcceptNum>
class wTCPCtrl
{
	public:
		...
		wTCPCtrl()
		{
			if(wServerCtrl::GetSingletonPtr()->mStatus == SERVER_STATUS_INIT)
			{
				Initialize();
			}
		...
		{
			W_ASSERT(vConnectNum >= 0 && vConnectNum < iMaxListenNum, return -99);
		...
		{
			W_ASSERT(vConnectNum >= 0 && vConnectNum < iMaxClientNum, return -99);
		...
		void CheckConnectStatus()
		{
			for(int i = 0; i < mAcceptConnectNum; i++) 
			{
				...
				wTCPClient<nRecvBufLen, nSendBufLen> *pAccept = &mAcceptConnect[iAcceptPos];
			...
			{
				wTCPListen *pListen = &mListenConnect[i];
			...
			{
				wTCPClient<nRecvBufLen, nSendBufLen> *pClient = &mClientConnect[i];
		...
		wTCPClient<nRecvBufLen, nSendBufLen> *GetClientConnect(int vClientNum)
		{
			W_ASSERT(vClientNum >= 0 && vClientNum < iMaxClientNum, return NULL);
			return &(mClientConnect[vClientNum]);
		}

		wTCPClient<nRecvBufLen, nSendBufLen> *GetAcceptConnect(int vAcceptNum)
		{
			W_ASSERT(vAcceptNum >= 0 && vAcceptNum < mAcceptConnectNum, return NULL);
			return &(mAcceptConnect[vAcceptNum]);
		}

		wTCPListen *GetListenConnect(int vListenNum)
		{
			W_ASSERT(vListenNum >= 0 && vListenNum < iMaxListenNum, return NULL);
	...
	private:
		...
		/**
		 * 添加一个accept的连接
		 * @param  vNewSocketFD [accept之后的socket fd]
		 * @param  vListenPos   [监听端口的位置]
		 * @param  vIPAddress   [description]
		 * @param  vPort        [description]
		 * @return              [description]
		 */
		int AddOneAcceptConn(int vNewSocketFD, int vListenPos, unsigned int vIPAddress, unsigned short vPort)
		{
			W_ASSERT(mAcceptConnectNum < iMaxAcceptNum && "too many connect now", return -1);
		...
		{
			W_ASSERT(mAcceptConnectNum > 0 && vAcceptCtrlPos < mAcceptConnectNum, return -1);
		...
		wTCPListen mListenConnect[iMaxListenNum];	// 所有的监听端口
		wTCPClient<nRecvBufLen, nSendBufLen> mClientConnect[iMaxClientNum];	// 所有的主动连接监听端口的连接
		wTCPClient<nRecvBufLen, nSendBufLen> mAcceptConnect[iMaxAcceptNum];	// 所有被listen端口监听所接受的端口
