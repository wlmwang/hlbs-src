 ...
 */
 ...
 /**
  *  实现了空闲队列数据结构，用于管理服务器实体
  */
#ifndef _W_FREE_LIST_H_
#define _W_FREE_LIST_H_
...
namespace W
{
	...
	class free_list
	{
		public:
			...
			T *get_one_node()
			{
				W_ASSERT(size_ >= 1 && size_ <= capacity() && "get_one_node failed", return NULL);

#ifdef _DEBUG_
				// 这种情况在理论上是不可能出现的
				W_ASSERT(first_ >= 0, return NULL);
#endif
...
#ifdef _DEBUG_
					// 这种情况在理论上是不可能出现的
					W_ASSERT(next_first_num >= 0, return NULL);
#endif
			...
			{
				W_ASSERT(array_id_ >= 0 && array_id_ < capacity() && "free_one_node failed", return);

				W_ASSERT(size_ >= 0 && size_ < capacity() && "free_one_node failed", return);

				W_ASSERT(node_list_[array_id_].get_next() == -1 && node_list_[array_id_].get_prev() == -1 && first_ != array_id_ && "free_one_node is already free", return);
			...
			void free_one_node(T *object_id_)
			{
				...
				W_ASSERT(distance_address % sizeof(base_node<T>) == 0 && "free_one_node failed", return);
			...
			{
				W_ASSERT(array_id_ >= 0 && array_id_ < capacity() && "get_base_node failed", ;);
			...
			{
				W_ASSERT(array_id_ >= 0 && array_id_ < capacity() && "get_base_node failed", return NULL);
				W_ASSERT(node_list_[array_id_].get_next() == -1 && node_list_[array_id_].get_prev() == -1 && first_ != array_id_ && "get_used_node is already free", return NULL);
