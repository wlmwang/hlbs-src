...
void wServer<T>::Recv()
{
	...
	for (int i = 0 ; i < iRet ; i++)
	{
		...
		iFD = pTask->Socket()->FD();
		sockType = pTask->Socket()->SockType();
		sockStatus = pTask->Socket()->SockStatus();
		if (iFD == FD_UNKNOWN)
		{
		...
		if (sockType == SOCK_TYPE_LISTEN && sockStatus == SOCK_STATUS_LISTENED)
		{
		...
		else if (sockType == SOCK_TYPE_CONNECT && sockStatus == SOCK_STATUS_CONNECTED)
		{
			if (mEpollEventPool[i].events & EPOLLIN)
			{
				//套接口准备好了读取操作
...
void wServer<T>::CheckTimeout()
{
	...
	if (mTaskPool.size() > 0)
	{
		...
		{
			sockType = (*iter)->Socket()->SockType();
			sockStatus = (*iter)->Socket()->SockType();
			if (sockType == SOCK_TYPE_CONNECT && sockStatus == SOCK_STATUS_CONNECTED)
			{
				//心跳检测
